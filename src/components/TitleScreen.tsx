...
const SUMMARY: Record<Difficulty, string> = {
  easy: '느리게 · 판정 넉넉 · 라이프 5',
  normal: '기준',
  hard: '빠르게 · 판정 빡빡 · 라이프 2',
};

export function TitleScreen({ best, onStart }: TitleScreenProps) {
  return (
    <div className="bg-field absolute inset-0 flex flex-col items-center justify-center gap-10 px-6 font-mono">
      <div className="text-center">
        <p className="text-base tracking-[0.35em] text-white/45">CHEAP MOSDAY</p>
        ...
        {DIFFICULTIES.map((difficulty) => (
          ...
          >
            <span className="flex items-baseline justify-between gap-4">
              <span className="text-3xl tracking-[0.15em]">{DIFFICULTY_LABEL[difficulty]}</span>
      ...
      <p className="text-center text-base text-white/40">
        큐가 판정선에 닿는 순간 누르세요
        <span className="key-hint mt-2 block tracking-[0.2em] text-white/30">
          ← A F&nbsp; / &nbsp;→ D J
        </span>
      </p>
    </div>
